import { TpsArchetypeArticulationExpectations, TpsArchetypeProfiles, TpsArchetypeRhythmProfiles, TpsArchetypeVolumeExpectations, TpsDiagnosticCodes, TpsSpec } from "./constants.js";
import { createWarningDiagnostic } from "./diagnostics.js";
const archetypeProfiles = TpsArchetypeProfiles;
const archetypeRhythmProfiles = TpsArchetypeRhythmProfiles;
export function appendArchetypeDiagnostics(targets, lineStarts, diagnostics) {
...
function appendProfileWarnings(target, archetype, profile, spokenWords, lineStarts, diagnostics) {
    ...
    if (articulationConflict) {
        diagnostics.push(createWarningDiagnostic(TpsDiagnosticCodes.archetypeArticulationMismatch, buildArticulationMessage(archetype, target.block.name, articulationConflict.metadata.articulationStyle ?? "unknown", profile.articulation), target.rangeStart, target.rangeEnd, lineStarts, buildArticulationSuggestion(profile.articulation)));
    }
    ...
    if (energyConflict && typeof energyConflict.metadata.energyLevel === "number") {
        diagnostics.push(createWarningDiagnostic(TpsDiagnosticCodes.archetypeEnergyMismatch, `Archetype '${archetype}' expects energy between ${profile.energy.min} and ${profile.energy.max}, but block '${target.block.name}' uses ${energyConflict.metadata.energyLevel} on '${energyConflict.cleanText}'.`, target.rangeStart, target.rangeEnd, lineStarts, `Keep [energy:N] between ${profile.energy.min} and ${profile.energy.max} for this archetype.`));
    }
    ...
    if (melodyConflict && typeof melodyConflict.metadata.melodyLevel === "number") {
        diagnostics.push(createWarningDiagnostic(TpsDiagnosticCodes.archetypeMelodyMismatch, `Archetype '${archetype}' expects melody between ${profile.melody.min} and ${profile.melody.max}, but block '${target.block.name}' uses ${melodyConflict.metadata.melodyLevel} on '${melodyConflict.cleanText}'.`, target.rangeStart, target.rangeEnd, lineStarts, `Keep [melody:N] between ${profile.melody.min} and ${profile.melody.max} for this archetype.`));
    }
    const volumeConflict = spokenWords.find((word) => isVolumeMismatch(word.metadata.volumeLevel, profile.volume));
    if (volumeConflict) {
        diagnostics.push(createWarningDiagnostic(TpsDiagnosticCodes.archetypeVolumeMismatch, buildVolumeMessage(archetype, target.block.name, volumeConflict.metadata.volumeLevel ?? "default", profile.volume), target.rangeStart, target.rangeEnd, lineStarts, buildVolumeSuggestion(profile.volume)));
    }
    ...
    if (speedConflict) {
        ...
        diagnostics.push(createWarningDiagnostic(TpsDiagnosticCodes.archetypeSpeedMismatch, `Archetype '${archetype}' expects inline speed changes to stay between ${profile.speed.min} and ${profile.speed.max} WPM, but block '${target.block.name}' reaches ${effectiveWpm} WPM on '${speedConflict.cleanText}'.`, target.rangeStart, target.rangeEnd, lineStarts, `Prefer inline speed tags that keep this scope between ${profile.speed.min} and ${profile.speed.max} WPM.`));
    }
}
function appendRhythmWarnings(target, archetype, rhythm, spokenWords, lineStarts, diagnostics) {
    ...
    pushRhythmWarning(diagnostics, TpsDiagnosticCodes.archetypeRhythmPhraseLength, target, lineStarts, !isWithinRange(metrics.averagePhraseLength, rhythm.phraseLength), `Archetype '${archetype}' expects average phrase length between ${rhythm.phraseLength.min} and ${rhythm.phraseLength.max} words, but block '${target.block.name}' averages ${formatMetric(metrics.averagePhraseLength)}.`, `Break phrases so this scope averages between ${rhythm.phraseLength.min} and ${rhythm.phraseLength.max} words.`);
    pushRhythmWarning(diagnostics, TpsDiagnosticCodes.archetypeRhythmPauseFrequency, target, lineStarts, !isWithinRange(metrics.pauseFrequencyPer100Words, rhythm.pauseFrequencyPer100Words), `Archetype '${archetype}' expects ${rhythm.pauseFrequencyPer100Words.min} to ${rhythm.pauseFrequencyPer100Words.max} pauses per 100 words, but block '${target.block.name}' has ${formatMetric(metrics.pauseFrequencyPer100Words)}.`, `Adjust pause markers so this scope lands between ${rhythm.pauseFrequencyPer100Words.min} and ${rhythm.pauseFrequencyPer100Words.max} pauses per 100 words.`);
    pushRhythmWarning(diagnostics, TpsDiagnosticCodes.archetypeRhythmPauseDuration, target, lineStarts, typeof metrics.averagePauseDurationMs === "number" && !isWithinRange(metrics.averagePauseDurationMs, rhythm.averagePauseDurationMs), `Archetype '${archetype}' expects average pause duration between ${rhythm.averagePauseDurationMs.min} and ${rhythm.averagePauseDurationMs.max} ms, but block '${target.block.name}' averages ${formatMetric(metrics.averagePauseDurationMs)} ms.`, `Adjust explicit pauses so this scope averages between ${rhythm.averagePauseDurationMs.min} and ${rhythm.averagePauseDurationMs.max} ms.`);
    pushRhythmWarning(diagnostics, TpsDiagnosticCodes.archetypeRhythmEmphasisDensity, target, lineStarts, !isWithinRange(metrics.emphasisDensityPercent, rhythm.emphasisDensityPercent), `Archetype '${archetype}' expects emphasis density between ${rhythm.emphasisDensityPercent.min}% and ${rhythm.emphasisDensityPercent.max}%, but block '${target.block.name}' is ${formatMetric(metrics.emphasisDensityPercent)}%.`, `Add or remove emphasis so this scope lands between ${rhythm.emphasisDensityPercent.min}% and ${rhythm.emphasisDensityPercent.max}%.`);
    pushRhythmWarning(diagnostics, TpsDiagnosticCodes.archetypeRhythmSpeedVariation, target, lineStarts, !isWithinRange(metrics.speedVariationPer100Words, rhythm.speedVariationPer100Words), `Archetype '${archetype}' expects ${rhythm.speedVariationPer100Words.min} to ${rhythm.speedVariationPer100Words.max} inline speed changes per 100 words, but block '${target.block.name}' has ${formatMetric(metrics.speedVariationPer100Words)}.`, `Adjust inline speed tags so this scope lands between ${rhythm.speedVariationPer100Words.min} and ${rhythm.speedVariationPer100Words.max} changes per 100 words.`);
}
function collectScopeMetrics(block, spokenWords, phraseWordCounts) {
...
function pushRhythmWarning(diagnostics, code, target, lineStarts, condition, message, suggestion) {
    if (!condition) {
        return;
    }
...
function isArticulationMismatch(value, expectation) {
...
function isVolumeMismatch(value, expectation) {
...
function hasInlineSpeedVariation(word, inheritedWpm) {
    return resolveEffectiveWordWpm(word, inheritedWpm) !== inheritedWpm;
}
function isSpeedMismatch(word, inheritedWpm, range) {
...
function resolveEffectiveWordWpm(word, inheritedWpm) {
...
function buildArticulationMessage(archetype, blockName, actual, expectation) {
    if (expectation === TpsArchetypeArticulationExpectations.neutral) {
...
function buildArticulationSuggestion(expectation) {
    if (expectation === TpsArchetypeArticulationExpectations.neutral) {
...
function buildVolumeMessage(archetype, blockName, actual, expectation) {
    if (expectation === TpsArchetypeVolumeExpectations.defaultOnly) {
...
function buildVolumeSuggestion(expectation) {
    if (expectation === TpsArchetypeVolumeExpectations.defaultOnly) {
...
//# sourceMappingURL=archetype-analysis.js.map