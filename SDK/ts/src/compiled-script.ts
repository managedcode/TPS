import type { CompiledBlock, CompiledPhrase, CompiledScript, CompiledSegment, CompiledWord, WordMetadata } from "./models.js";

export function normalizeCompiledScript(script: CompiledScript): CompiledScript {
...
export function parseCompiledScriptJson(json: string): CompiledScript {
  if (!json.trim()) {
    throw new TypeError("Compiled TPS JSON must be a non-empty string.");
  }

  const parsed = JSON.parse(json) as CompiledScript | null;
...
export function validateCompiledScript(script: CompiledScript): void {
...
function normalizeSegment(segment: CompiledSegment, wordById: ReadonlyMap<string, CompiledWord>): CompiledSegment {
...
function normalizeBlock(block: CompiledBlock, wordById: ReadonlyMap<string, CompiledWord>): CompiledBlock {
...
function normalizePhrase(phrase: CompiledPhrase, wordById: ReadonlyMap<string, CompiledWord>): CompiledPhrase {
...
function cloneWord(word: CompiledWord): CompiledWord {
  return deepFreeze({
    id: word.id,
    index: word.index,
    kind: word.kind,
...
function cloneWordMetadata(metadata: WordMetadata): WordMetadata {
  return {
...
function validateWords(words: CompiledWord[], seenIds: Set<string>): void {
  let previousWord: CompiledWord | undefined;
...
function validateCanonicalScopeWords(
  scope: string,
  ownerId: string,
  scopeWords: CompiledWord[],
  startWordIndex: number,
  endWordIndex: number,
  startMs: number,
  endMs: number,
  canonicalWords: CompiledWord[],
...
function validateWordReferences(words: CompiledWord[], segmentIds: ReadonlySet<string>, blockIds: ReadonlySet<string>, phraseIds: ReadonlySet<string>): void {
...
function validateIdentifier(id: string, scope: string, seen: Set<string>): void {
  if (!id.trim()) {
...
function getNormalizedWord(id: string, wordById: ReadonlyMap<string, CompiledWord>): CompiledWord {
...
function assertRecord(value: unknown, message: string): asserts value is Record<string, string> {
