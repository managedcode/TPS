...
import type { CompiledPhrase, CompiledWord, TpsDiagnostic, WordMetadata } from "./models.js";
...
interface LiteralScope {
  name: string;
}

interface ActiveInlineState {
  emotion: string;
  inlineEmotion?: string;
  speaker?: string;
  emphasisLevel: number;
  ...
  stressGuide?: string;
  stressWrap: boolean;
  hasAbsoluteSpeed: boolean;
  absoluteSpeed: number;
  hasRelativeSpeed: boolean;
  relativeSpeedMultiplier: number;
}
...
export function compileContent(
  ...
  const currentPhrase: WordSeed[] = [];
  const scopes: InlineScope[] = [];
  const literalScopes: LiteralScope[] = [];
  let builder = "";
  let token: TokenAccumulator | undefined;
  ...
  for (let index = 0; index < protectedText.length; index += 1) {
    ...
    if (currentCharacter === "[") {
      ...
      if (!tag) {
        diagnostics.push(
          createDiagnostic(
            TpsDiagnosticCodes.unterminatedTag,
  ...
  for (const scope of scopes) {
    diagnostics.push(
      createDiagnostic(
        TpsDiagnosticCodes.unclosedTag,
...
function handleTagToken(
  tag: TagToken,
  literalScopes: LiteralScope[],
  scopes: InlineScope[],
  words: WordSeed[],
  phrases: PhraseSeed[],
  currentPhrase: WordSeed[],
  inherited: InheritedFormattingState,
  ...
  if (tag.name === TpsTags.editPoint) {
    if (tag.argument && !TpsSpec.editPointPriorities.includes(tag.argument as never)) {
...
function createScope(
  ...
  if (TpsSpec.volumeLevels.includes(tag.name as never)) {
    return { name: tag.name, volumeLevel: tag.name };
  }

  if (TpsSpec.deliveryModes.includes(tag.name as never)) {
...
function tryHandleMarkdownMarker(text: string, index: number, scopes: InlineScope[]): boolean {
  if (text[index] !== "*") {
    return false;
  }
...
function readTagToken(text: string, index: number): TagToken | undefined {
...
function requiresTokenBoundary(tagName: string): boolean {
  return [TpsTags.pause, TpsTags.breath, TpsTags.editPoint].includes(tagName as never);
}

function tryHandleSlashPause(
  text: string,
  index: number,
  builder: string,
  token: TokenAccumulator | undefined
): boolean {
...
function finalizeToken(
  ...
  if (!builder || !token) {
    return { builder: "", token: undefined as TokenAccumulator | undefined };
  }
...
function attachStandalonePunctuation(words: WordSeed[], currentPhrase: WordSeed[], punctuation: string): boolean {
...
function flushPhrase(phrases: PhraseSeed[], currentPhrase: WordSeed[]): void {
...
function createControlWord(kind: "pause" | "breath" | "edit-point", inherited: InheritedFormattingState, pauseDurationMs?: number, editPointPriority?: string): WordSeed {
  const metadata: WordMetadata = {
    ...
    headCue: TpsSpec.emotionHeadCues[inherited.emotion as keyof typeof TpsSpec.emotionHeadCues]
  };
...
function resolveActiveState(scopes: InlineScope[], inherited: InheritedFormattingState): ActiveInlineState {
...
function isPairedScope(tagName: string): boolean {
  return ![TpsTags.pause, TpsTags.breath, TpsTags.editPoint].includes(tagName as never);
}

function isSpokenWord(word: WordSeed): boolean {
...
class TokenAccumulator {
  private readonly stressText: string[] = [];
  ...
  private hasAbsoluteSpeed = false;
  private absoluteSpeed = 0;
  private hasRelativeSpeed = false;
  private relativeSpeedMultiplier = 1;
  private speaker?: string;

  public apply(state: ActiveInlineState, character: string): void {
  ...
  public buildWordMetadata(inheritedWpm: number): WordMetadata {
    const metadata: WordMetadata = {
      ...
      headCue: TpsSpec.emotionHeadCues[(this.emotionHint || TpsSpec.defaultEmotion) as keyof typeof TpsSpec.emotionHeadCues]
    };
